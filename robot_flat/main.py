...
async def main() -> None:
    ...
    cfg = FlatConfig(
        ...
        range_pct=get_env_float("RANGE_PCT", 12.0),
        ...
        order_levels=get_env_list("ORDER_LEVELS", [-3.0, -5.0, -8.0]),
        stop_zone_pct=get_env_float("STOP_ZONE_PCT", 3.0),
        stop_from_border_pct=get_env_float("STOP_FROM_BORDER_PCT", 8.0),
        tp_offset_pct=get_env_float("TP_OFFSET_PCT", 12.0),
        max_positions=get_env_int("MAX_POSITIONS", 3),
        poc_recalc_every=get_env_int("POC_RECALC_EVERY", 0),
        partial_close_pct=get_env_float("PARTIAL_CLOSE_PCT", 50.0),
        trailing_after_poc_pct=get_env_float("TRAILING_AFTER_POC_PCT", 2.0),
        trend_filter_enabled=get_env_int("TREND_FILTER_ENABLED", 1) == 1,
