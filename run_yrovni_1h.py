"""Запуск VP-скринера на 1 час + сохранение отчёта."""
...
def _generate_report(all_signals: list, scan_count: int, elapsed: float) -> None:
    ...
    with open(filepath, "w", encoding="utf-8") as f:
        ...
        if not all_signals:
            ...
            # Группируем по символу, берём лучшие уровни
            by_symbol: dict = {}
...
async def main() -> None:
    setup_logging("logs/yrovni_1h.log", "INFO")
    cfg = _load_config()
    metrics = ScreenerMetrics()

    logger.info("VP-скринер на 1 час: interval=%ss, min_turnover=$%sM",
        ...
        try:
            ...
            for symbol, _ in filtered:
                ...
                if result.poc_levels or result.daily_levels:
                    ...
                    logger.info("СИГНАЛ: %s poc=%d daily=%d",
                                result.symbol, len(result.poc_levels), len(result.daily_levels))
